...
本指南面向“玩家”，不涉及开发细节。规则口径以 `docs/game-rules-spec.md` 为准。
...
![建局界面标注](assets/player-guide/01-setup-annotated.jpg)
...
![多人建局标注](assets/player-guide/02-add-players-annotated.jpg)
...
![棋盘总览标注](assets/player-guide/03-board-overview-annotated.jpg)
...
![回合动作标注](assets/player-guide/04-roll-and-actions-annotated.jpg)
...
![卡牌弹窗标注](assets/player-guide/05-card-modal-annotated.jpg)
...
![市场响应窗口标注](assets/player-guide/06-market-response-window-annotated.jpg)
...
![合资项目标注](assets/player-guide/07-joint-venture-section-annotated.jpg)
...
![玩家借贷标注](assets/player-guide/08-player-loan-section-annotated.jpg)
...
![资产负债报表标注](assets/player-guide/10-balance-sheet-panel-annotated.jpg)
...
![游戏日志标注](assets/player-guide/11-game-log-panel-annotated.jpg)
...
![回放导入标注](assets/player-guide/09-replay-import-section-annotated.jpg)
...
### Q2：现金不够还能买/付吗？
内圈（老鼠赛跑）允许银行融资覆盖差额（会生成负债并写日志）；外圈（快车道）禁止银行贷款，现金不足只能放弃/进入清算流程（具体以规则规范为准）。

### Q3：这局的胜利目标是什么？
内圈目标是让被动收入超过总支出以解锁“进入快车道”；进入快车道后以达成快车道目标为胜利（详见 `docs/game-rules-spec.md` 的外圈章节）。
